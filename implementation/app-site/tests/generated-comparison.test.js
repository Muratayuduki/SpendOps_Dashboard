...
test("single-screen markup has every element required by the script", () => {
  ...
  assert.match(html, /id="load-saved"[^>]*>分析を開く</);
  assert.doesNotMatch(html, />保存済み分析</);
  assert.match(html, /みんなと比べる/);
  assert.match(html, /自分の過去と比べる/);
  assert.doesNotMatch(html, /<span class="switch-label">/);
  assert.match(html, /role="group" aria-label="比べ方"/);
  assert.match(html, /role="group" aria-label="見る範囲の切替"/);
  assert.match(html, /id="personal-mode"[^>]*aria-pressed="false"/);
  assert.doesNotMatch(html, /comparison-card-state/);
  ...
  assert.match(css, /\.comparison-switch, \.scope-switch \{[^}]*display:\s*flex[^}]*padding:\s*3px/s);
  ...
  assert.match(css, /\.comparison-switch button\.is-active, \.scope-switch button\.is-active \{[^}]*background:\s*var\(--lime\)/s);
  ...
  assert.match(css, /\.category-layout \{[^}]*grid-template-columns:\s*105px 1fr[^}]*gap:\s*13px/s);
  assert.match(css, /\.donut \{[^}]*width:\s*94px[^}]*height:\s*94px/s);
  assert.match(css, /\.category-list \{[^}]*gap:\s*7px/s);
  assert.match(css, /@media \(max-width: 900px\)[\s\S]*\.analysis-grid \{[^}]*height:\s*auto/s);
  assert.match(css, /@media \(max-width: 540px\)[\s\S]*\.category-layout \{[^}]*grid-template-columns:\s*90px 1fr/s);
  assert.match(script, /`比較対象：\$\{comparisonTarget\}`/);
  assert.match(script, /`\$\{comparisonTarget\}との差：\$\{formatRate\(summary\.difference_rate\)\}`/);
  assert.doesNotMatch(script, /\.comparison-card-state span/);
  assert.match(html, /id="open-transactions"/);
  assert.match(html, /id="open-category-review"/);
  assert.match(html, /id="open-transactions"[^>]*>明細を見る</);
  assert.match(html, /id="open-category-review"[^>]*>分類を見直す</);
  assert.doesNotMatch(html, /class="detail-actions"/);
  assert.match(html, /class="heading-status"[\s\S]*id="dataset-badge"[\s\S]*id="comparison-status"/);
  assert.match(html, /id="dataset-badge" hidden><\/span>/);
  assert.match(html, /id="comparison-status" hidden><\/span>/);
  assert.match(css, /\.result-heading \{[^}]*display:\s*flex[^}]*gap:\s*12px/s);
  assert.match(css, /\.result-heading h2 \{[^}]*white-space:\s*nowrap/s);
  assert.match(css, /\.report-title-row \{[^}]*flex-wrap:\s*nowrap/s);
  assert.match(css, /\.comparison-switch button \{[^}]*display:\s*flex[^}]*min-width:\s*126px/s);
  assert.match(css, /\.detail-button \{[^}]*min-height:\s*31px/s);
  assert.match(html, /id="transaction-title">支払い明細</);
  assert.doesNotMatch(html, /支払い明細・使いみちの編集/);
  assert.match(script, /querySelector\("#comparison-status"\)\.hidden = true/);
  assert.match(script, /badge\.hidden = true/);
  ...
  assert.equal(reportPanel.indexOf('id="comparison-control"') < reportPanel.indexOf('id="open-transactions"'), true);
});
