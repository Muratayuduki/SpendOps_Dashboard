...
test("analysis insight explains amounts, comparisons, changes, concentration, and next action", () => {
  ...
  assert.match(insight, /10,000円（5件）/);
  assert.match(insight, /他者平均より2,000円（25\.0%）高い/);
  assert.match(insight, /食費の6,000円（60\.0%）/);
  assert.match(insight, /前月より1,000円（11\.1%）増加/);
  assert.match(insight, /PayPayが支払い方法の60\.0%/);
  assert.match(insight, /基準超過2,000円分の上位明細/);
  assert.match(insight, /二重計上/);
});
...
test("analysis insight states why comparison is unavailable and marks partial data", () => {
  ...
  assert.match(insight, /暫定値/);
  assert.match(insight, /匿名化に必要な人数/);
...
test("provided category aliases and expanded merchant rules reduce other classifications", () => {
  ...
  assert.equal(core.classifyCategory("AMAZON.CO.JP", ""), "日用品");
...
test("cloud upload payload contains only monthly aggregates", () => {
  const analysis = core.buildLocalAnalysis([
    ["取引日", "出金金額", "取引先"],
  ...
  assert.equal(serialized.includes("秘密の店舗"), false);
...
test("stored monthly summaries rebuild personal comparisons without raw details", () => {
  ...
  assert.equal(report.privacy.includes("原本CSVと明細は保存していません"), true);
});
