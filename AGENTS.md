...
プロジェクト固有のカスタムエージェント定義は`.codex/agents/`に置く。ユーザーの依頼が次の担当範囲に一致する場合は、原則として対応するカスタムエージェントへ委譲し、rootが結果を統合する。機能実装は`implementation/`、資料はリポジトリ外の`C:\development\SpendOps_Dashboard_Material\`、プロンプト・ガードレール・引継ぎは`project-guidance/`に置く。
